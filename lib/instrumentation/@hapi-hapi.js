...
const instrumentHapi = function(Hapi, opts = {}) {
  ...
  const wrapper = function(...args) {
    ...
    // the Hapi toolkit?
    server.decorate("toolkit", "beeline", { api });
    ...
    // Once request life cycle is at the point of parsed query / params, add them
    ...
    server.events.on({ name: "request", channels: "error" }, (request, event, _tags) => {
      ...
      tracker.setTracked(tracked);
      if (error) {
        api.addContext({ error: { message: error.message, stack: error.stack } });
      } else {
    ...
    function instrumentRoute(original) {
      return function(routeOptions) {
        ...
        // Hapi exposes six extension points [onRequest, onPreAuth, onCredentials, onPostAuth, onPreHandler, onPostHandler, onPreResponse]
        // that can be configured on a route-by-route basis. Each handler is independently
