...
const instrumentLoad = (exports.instrumentLoad = (mod, loadRequest, parent, opts = {}) => {
  ...
  let instrumentation = require(instrumentationPath(opts.injected ? "inject" : loadRequest));
  let new_mod;
  ...
  if (!opts.disableInstrumentationOnLoad) {
    ...
    shimmer.wrap(Module, "_load", function(original) {
      return function(request, parent, isMain) {
        ...
        let optsForInstrumentation = opts[request] || {};
        if (opts.inject && opts.inject.has(request)) {
          return instrumentLoad(mod, request, parent, opts.inject && opts.inject.get(request));
        } else {
          return instrumentLoad(mod, request, parent, optsForInstrumentation);
        }
      };
    });
  }
